...
class FortinetL3ServicePlugin(router.L3RouterPlugin):
    ...
    def update_router(self, context, id, router):
        LOG.debug(_("update_router: id=%s, router=%s" % (id, router)))
    ...
    def delete_floatingip(self, context, id):
        LOG.debug(_("delete_floatingip: id=%s" % id))
    ...
    def _add_interface_by_subnet(self, context, router, subnet_id, owner):
        LOG.debug(_("_add_interface_by_subnet: router=%s, subnet_id=%s, "
                    "owner=%s" % (router, subnet_id, owner)))
